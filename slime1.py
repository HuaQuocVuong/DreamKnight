...
from slime1_animation import Slime1AnimationLoader

# ================================================================================================
# CLASS SLIME1 — Kẻ địch Slime 1 (chậm nhất, ít máu nhất, luôn đuổi theo player)
# ================================================================================================
...
class Slime1(pygame.sprite.Sprite):
    ...
    def __init__(self, x, y, scale_factor=2.0):
        """
        Khởi tạo Slime1 với animation, AI, máu, âm thanh
        x, y: vị trí spawn
        ...
        all_anims = Slime1AnimationLoader.load_all(scale_factor)
        ...
        self.attack_range    = 60      # Phạm vi kích hoạt tấn công (px) — xa nhất
        self.attack_duration = 550     # Thời gian animation tấn công (ms)
        ...
        # Load âm thanh (tấn công, bị thương, chết) — dùng thư mục riêng slime1
    ...
    def take_damage(self, damage) -> bool:
        ...
        self.health -= damage
        print(f"[Slime1] Nhận {damage} sát thương! Máu còn: {self.health}/{self.max_health}")
    ...
    def _handle_chase(self, target_x, target_y):
        ...
        if self.state == "run":
            speed = RUN_SPEED * self.run_speed_multiplier      # 0.4 lần tốc độ chạy
        else:
            speed = PLAYER_SPEED * self.walk_speed_multiplier  # 0.3 lần tốc độ đi bộ
    ...
    def _start_attack(self, current_time):
        ...
        print(f"[Slime1] Bắt đầu tấn công (dist <= {self.attack_range}px)")
    ...
    def draw(self, screen, camera):
        ...
        # Vị trí home đã bị comment — không vẽ debug home
