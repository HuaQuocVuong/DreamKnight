...
# ================================================================================================
# CLASS PLANT2 — Kẻ địch Plant 2 (máu trung bình, sát thương khá, luôn đuổi theo player)
# ================================================================================================
...
class Plant2(pygame.sprite.Sprite):
    ...
    def __init__(self, x, y, scale_factor=2.0):
        """
        Khởi tạo Plant2 với animation, AI, máu, âm thanh
        x, y: vị trí spawn
        ...
        # Hệ số tốc độ — ngang Slime3
        self.run_speed_multiplier  = 0.5   # Tốc độ chạy = RUN_SPEED * 0.5
        self.walk_speed_multiplier = 0.4   # Tốc độ đi bộ = PLAYER_SPEED * 0.4
        ...
        # Chỉ số máu và sát thương — trung bình khá
        self.health         = 350      # Máu hiện tại
        self.contact_damage = 30       # Sát thương khi player chạm vào
        self.max_health     = 350      # Máu tối đa
        ...
        # Load âm thanh (tấn công, bị thương, chết) — dùng chung với slime3
    ...
    def update(self, delta_time, map_width, map_height):
        ...
        plant_cx         = self.x + self.width  // 2   # Tâm plant
        plant_cy         = self.y + self.height // 2

        dist_player_to_home = math.hypot(px - home_cx,   py - home_cy)  # Khoảng cách player → home
        dist_to_player      = math.hypot(plant_cx - px,  plant_cy - py) # Khoảng cách plant → player
    ...
    # Cập nhật hướng nhìn (direction) dựa vào góc giữa plant và player
    ...
    def _handle_chase(self, target_x, target_y):
        ...
        if self.state == "run":
            speed = RUN_SPEED * self.run_speed_multiplier      # 0.5 lần tốc độ chạy
        else:
            speed = PLAYER_SPEED * self.walk_speed_multiplier  # 0.4 lần tốc độ đi bộ
    ...
    def _start_attack(self, current_time):
        ...
        print(f"[Plant2] Bắt đầu tấn công (dist <= {self.attack_range}px)")

    # Kết thúc tấn công: gây 12 sát thương nếu player còn trong phạm vi
    def _end_attack(self):
        self.state        = "idle" if not self.is_running else "run"
        self.is_attacking = False
        if self.direction in self.attack_anims:
            self.attack_anims[self.direction].reset()
        # Gây 12 sát thương (trung bình)
    ...
    # ------------------------------------------------------------------
    # VẼ — Render plant lên màn hình (có debug nếu bật)
    # ------------------------------------------------------------------
    ...
    def draw(self, screen, camera):
        ...
        # Hitbox (xanh lá nhạt) + phạm vi tấn công (cam)
        pygame.draw.circle(screen, (50, 205, 50), (cx, cy), self.body_radius, 2)
